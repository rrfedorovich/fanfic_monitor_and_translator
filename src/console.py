...
def main():
    ...
    if ENV.load_env():  # Если заданы все значения.
        ...
        fanfics_or_None = [
            FanficFactory.get_fanfic(*row) for row in csv_base.read_csv()
        ]
        ...
        for index_ff, ff in enumerate(fanfics):
            ...
            if new_chapters:
                for chapter in new_chapters:
                    translated = tr.run(
                        chapter.title, chapter.text, f"Перевод главы {chapter.number}."
                    )
                ...
                if not error_flag:
                    epub_chapters = [
                        ChapterEpub(ch.title, ch.text) for ch in new_chapters
                    ]
